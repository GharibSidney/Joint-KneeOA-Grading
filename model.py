...
class MILAggregator_ORG(nn.Module):
...
class CompleteMILModel_ORG(nn.Module):
    def __init__(self, feature_extractor_out_dim, num_classes, aggregation_type='attention'):
        ...
        self.aggregator = MILAggregator_ORG(input_embedding_dim=feature_extractor_out_dim,
...
class CompleteMILModel(nn.Module):
    ...
    def forward(self, list_of_patch_bags): # list_of_patch_bags: list of tensors, each (N_i, C, H, W)
        ...
        final_batch_logits = torch.cat(batch_logits, dim=0) # (batch_size, num_classes)
        valid_att_scores = [x for x in batch_att_scores if x is not None]
        if valid_att_scores:
            final_batch_att_scores = torch.cat(valid_att_scores, dim=0) # (batch_size, num_patches)
        else:
